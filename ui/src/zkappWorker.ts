...
const functions = {
    ...
    createGuessProof: async (args: { signedData: SignedData; unseparatedGuess: number }) => {
        const signature = Signature.fromBase58(args.signedData.signature);
    ...
    getZkAppStates: async () => {
        ...
        await fetchAccount({ publicKey });
        const [codeMasterId, codeBreakerId, rewardFinalizeSlot, turnCountMaxAttemptsIsSolved] =
            await Promise.all([
        ...
        const { rewardAmount, finalizeSlot } = separateRewardAndFinalizeSlot(rewardFinalizeSlot);
        let [turnCount, maxAttempts, isSolved] = separateTurnCountAndMaxAttemptSolved(
    ...
    submitGameProof: async () => {
        const transaction = await Mina.transaction(async () => {
            await state.zkappInstance!.submitGameProof(state.lastProof as StepProgramProof);
        });
        state.transaction = transaction;
    },
