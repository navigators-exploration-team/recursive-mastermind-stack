...
interface MinaWallet {
    ...
    signFields: (args: { message: Array<string | number> }) => Promise<SignedData | ProviderError>;
...
export const useZkAppStore = defineStore("useZkAppModule", {
    ...
    actions: {
        ...
        async joinGame(gameId?: string) {
            // @ts-ignore
            this.webSocketInstance = new WebSocketService(gameId ?? (this.zkAppAddress as string));
        ...
        async createInitGameTransaction(
            ...
            try {
                ...
                this.zkAppAddress = await this.zkappWorkerClient!.createInitGameTransaction(
        ...
        async acceptGame() {
            try {
                this.loading = true;
                this.stepDisplay = "Creating a transaction...";
                await this.zkappWorkerClient!.createAcceptGameTransaction(this.publicKeyBase58);
                this.stepDisplay = "Creating proof...";
                await this.zkappWorkerClient!.proveTransaction();
                this.stepDisplay = "Getting transaction JSON...";
                const transactionJSON = await this.zkappWorkerClient!.getTransactionJSON();
        ...
        async claimRewardTransaction() {
            try {
                this.loading = true;
                this.stepDisplay = "Creating a transaction...";
                await this.zkappWorkerClient!.createClaimRewardTransaction(this.publicKeyBase58);
                this.stepDisplay = "Creating proof...";
                await this.zkappWorkerClient!.proveTransaction();
                this.stepDisplay = "Getting transaction JSON...";
                const transactionJSON = await this.zkappWorkerClient!.getTransactionJSON();
        ...
        async getRole() {
            this.userRole = await this.zkappWorkerClient!.getUserRole(this.publicKeyBase58);
        },
    },
});
