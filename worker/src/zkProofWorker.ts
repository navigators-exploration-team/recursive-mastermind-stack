...
import { MastermindZkApp,StepProgram } from '@navigators-exploration-team/mina-mastermind'; 
...
const proofWorker = new Worker('proofQueue', async (job:Job) => {

  try {  
    if(job.name === 'checkGameCreation'){
       await checkGameCreation(job)
